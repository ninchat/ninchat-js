	...
	webSocketReceive = function(s, ws, fail) {
		...
		/* while (true) { */ case 1:
			...
			/* if (!connected && event === ptrType.nil) { */ case 21:
				...
				/* } */ case 25:
				$r = $send(fail, new structType.ptr()); /* */ $s = 28; case 28: if($c) { $c = false; $r = $r.$blk(); } if ($r && $r.$blk !== undefined) { break s; }
				$s = -1; return [gotEvents, hostHealthy];
				return [gotEvents, hostHealthy];
			/* } */ case 22:
			...
			/* } else if (_selection$1[0] === 1) { */ case 31:
				...
				$r = $send(fail, new structType.ptr()); /* */ $s = 37; case 37: if($c) { $c = false; $r = $r.$blk(); } if ($r && $r.$blk !== undefined) { break s; }
				$s = -1; return [gotEvents, hostHealthy];
				return [gotEvents, hostHealthy];
...
$packages["ninchatclient/lib"] = (function() {
	...
	funcType$1 = $funcType([ptrType], [mapType], false);
	...
	Promise.ptr.prototype.invoke = function(logPrefix, callback, args) {
		...
		p = [p];
		p[0] = this;
		/* */ if (!(p[0].OnPanic === $throwNilPointerError)) { $s = 1; continue; }
		/* */ $s = 2; continue;
		/* if (!(p[0].OnPanic === $throwNilPointerError)) { */ case 1:
	...
	SessionAdapter.ptr.prototype.InvokeOnSessionEvent = function(logPrefix, callback, e) {
		...
		logPrefix = [logPrefix];
		adapter[0] = this;
		/* */ if (!(adapter[0].OnPanic === $throwNilPointerError)) { $s = 1; continue; }
		/* */ $s = 2; continue;
		/* if (!(adapter[0].OnPanic === $throwNilPointerError)) { */ case 1:
	...
	SessionAdapter.ptr.prototype.OnClose = function(callback) {
		...
		adapter.Session.OnClose = (function $b() {
			...
			/* */ if (!(adapter.OnPanic === $throwNilPointerError)) { $s = 1; continue; }
			/* */ $s = 2; continue;
			/* if (!(adapter.OnPanic === $throwNilPointerError)) { */ case 1:
	...
	SessionAdapter.ptr.prototype.OnLog = function(callback) {
		...
		adapter.Session.OnLog = (function $b(tokens) {
			...
			/* while (true) { */ case 3:
				...
				/* */ if ($assertType(_ref$1, $error, true)[1]) { $s = 6; continue; }
				/* */ $s = 7; continue;
				/* if ($assertType(_ref$1, $String, true)[1]) { */ case 5:
					t = _ref$1.$val;
					str = t;
					$s = 8; continue;
				/* } else if ($assertType(_ref$1, $error, true)[1]) { */ case 6:
					t$1 = _ref$1;
					_r = t$1.Error(); /* */ $s = 9; case 9: if($c) { $c = false; _r = _r.$blk(); } if (_r && _r.$blk !== undefined) { break s; }
					str = _r;
					$s = 8; continue;
				/* } else { */ case 7:
					t$2 = _ref$1;
	...
	newSession = function(dontRecover) {
		var $ptr, adapter, dontRecover, session;
		...
		if (!(dontRecover === undefined)) {
			adapter.OnPanic = $throwNilPointerError;
		}
